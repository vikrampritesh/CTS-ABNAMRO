...
public class FixedDepositAccountDto extends AccountDto {
    ...
    public boolean isMaturingDepositWithInMonth(Integer priorMonth) {
       if (maturityDate != null) {
           ...
           if (super.getMonthsCountFromOpening() == (this.getMonthsCountOpeningToMaturity()-priorMonth)) {
    ...
    @JsonIgnore
    public Integer getMonthsCountFromMaturity() {
        if (!StringUtils.isBlank(maturityDate)) {
            DateTimeFormatter parser = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            return Long.valueOf(ChronoUnit.MONTHS.between(YearMonth.from(LocalDate.parse(maturityDate, parser)),
                    YearMonth.from(LocalDate.parse(LocalDate.now().toString())))).intValue();
        }
        return null;
    }
